$(document).ready(function () {
    ...
    $('#CP_TopImage2ColWithCopy_7 strong.blue-text, #CP_TopImage2ColWithCopy_3 strong.blue-text, #CP_TopImage2ColWithCopy_4 strong.blue-text,#CP_TopImage2ColWithCopy_1 strong.blue-text,#CP_TopImage2ColWithCopy_2 strong.blue-text, #CP_TopImage2ColWithCopy_13 strong.blue-text,#CP_TopImage2ColWithCopy_6 strong.blue-text').each(function () {
        $(this).wrap("<span class='pt-0 db'></span>");
    });
    /*346226 */
...
$(window).on("load", function () {
    ...
    /*carousel buttons*/
    if (navigator.userAgent.toLowerCase().indexOf('iphone') > 0 || navigator.userAgent.toLowerCase().indexOf('mac') > 0) {
        $('.main-content .slick-slider ul.slick-dots li').removeAttr('role');
        $('.main-content .slick-slider ul.slick-dots').removeAttr('role');
    }
});
/*end*/
...
$(window).load(function () {
    ...
    $(".CP_FiveColumnsWithCopy_1 .bottom-content").css("clear", "both");
    var chatText = $(".fixed-support-links .static-support-links-3");
    if (chatText.find('a #lpChatButton').text() == "" || chatText.find('a #lpChatButton').text() == " ") {
        chatText.remove();
    }
    /*Footer component*/
    ...
    if (navigator.userAgent.toLowerCase().indexOf('iphone') > 0) {
        $('.cp-side-media-with-copy .ms-grid .ms-row.display-table').css('display', 'block');
    }
});


function slickarrowstate() {
...
/*MWF 1.53.0*/
$(window).load(function () {
    $(".cp-pricing-table-v1 .cp-pricing-table-container h2").each(function () {
        if ($(this).text() == "") {
            $(this).remove();
        }
    });
    // $("#CP_StaticMenu #static_menu_1 li a").keydown(function (e) {
    //     if (e.shiftKey && e.keyCode == 9) {
    //         setTimeout(function () {                  
    //               $("#CP_StaticMenu #static_menu_1 li a:first").attr("tabindex", "0").focus();                
    //         }, 100);
    //     }
    // });
    ...
    $(".popup-overlay .panel-popup .close-button-wrapper").keydown(function (e) {
        if (e.shiftKey && e.keyCode == 9) {
            $(".popup-overlay .panel-popup .content-container a:last").attr("tabindex", "0").focus();
        }
    })
});
