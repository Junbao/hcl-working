...
$(document).ready(function () {
    ...
    $('.cp-sticky-nav .tab:last-child').on({
        keydown: function (e) {
            if (e.shiftKey && e.keyCode == 9) {
                var currentIndex = $(this).index() - 1;
                $(this).siblings().eq(currentIndex).focus();
            } else if (e.keyCode == 9) {
                $('.cp-sticky-nav-sub').trigger('click');
                $('.cp-sticky-nav-sub').attr('data-collapse', 'true');
            }
        }
    });
...
$(document).ready(function (e) {
    ...
    $(".close-button, button[data-js-dialog-hide]").on("click", function (e) {
        e.preventDefault();
        $(".active-video").focus();
...
/* removed image video functionality 351197 
...
function changeSupToAnchor() {
    ...
    $('.supLink').each(function (i) {
        ...
        if(!$(this).hasClass('c-hyperlink')){
            $(this).addClass('c-hyperlink');    
        }
    ...
    $('.supFn').each(function () {
        ...
        if ($(this).text() === innerContent) {
            ...
            setTimeout(function () {
                ...
                referencedFnSup.parents('p.c-caption-1').attr('tabindex','0').focus();
            }, 500);
...
$(document).on('click', '.supLink', function (e) {
    ...
    if (innerContent == $('.lastClicked .supText').text() || innerContent == $('.lastClicked').text()) {
        ...
        innerContent.closest('p.c-caption-1').attr('tabindex', '-1');
    } else {
...
$(window).load(function () {
    ...
    $('.main-content p.caption').each(function () {
        if($(this).find('a').hasClass('supLink') && !$(this).find('a').hasClass('c-hyperlink')){
            $(this).find('a').addClass('c-hyperlink');
        }
    });
    // $('.supFn').each(function () {
...
$(document).ready(function () {
    $('.CP_AccordionWithTable .accordion-main-panel button').on('keydown', function (e) {
        if (e.keyCode == 13 || e.which == 13) {
            setTimeout(function () {
                $('.selectedacctab').focus();
            }, 500);
        }
    });
...
$(document).ready(function () {
    ...
    //changing images on hover state
    $('.cp-media-carousel-with-frames .carousel-frame a img[data-hover-image]').each(function(){
        var playIcon = $(this).attr('src');
        var hoverIcon = $(this).data('hover-image');
        var carouselLink = $(this).closest('a');
       
        carouselLink.on('mouseover', function(){
            $(this).find('img.play-button').attr('src', hoverIcon);
            $(this).find('picture:nth-of-type(2) source').attr('srcset', hoverIcon);
        });
    
        carouselLink.on('mouseout', function(){
            $(this).find('img.play-button').attr('src', playIcon);
            $(this).find('picture:nth-of-type(2) source').attr('srcset', playIcon);
        });
        
        carouselLink.on('focus', function(){
            $(this).find('img.play-button').attr('src', hoverIcon);
            $(this).find('picture:nth-of-type(2) source').attr('srcset', hoverIcon);
        });
        
        carouselLink.on('blur', function(){
            $(this).find('img.play-button').attr('src', playIcon);
            $(this).find('picture:nth-of-type(2) source').attr('srcset', playIcon);
        });
    });
    
    $('.cp-media-carousel-with-frames .carousel-navigation img').on('click', function(){
        //changing images on hover state
        $('.cp-media-carousel-with-frames .carousel-frame a img[data-hover-image]').each(function(){
            var playIcon = $(this).attr('src');
            var hoverIcon = $(this).data('hover-image');
...
$(window).resize(function () {
    ...
    if (window.matchMedia("(max-width: 767px)").matches) {
        $('.c-back-to-top').removeClass('c-back-to-top').addClass('m-back-to-top');
    } else {
        $('.c-back-to-top').removeClass('m-back-to-top').addClass('c-back-to-top');
    }
});
...
$(document).ready(function () {
    ...
    $('.accordion-main-panel .subitems .mobile_table_header:last-child').on('keydown', function (e) {
        if (e.keyCode == 9) {
            $('.selectedacctab-section .selectedacctab').trigger('click');
        }
    });
    $(".cp-sticky-nav nav ul li a").each(function () {
        if ($(this).attr('data-active') == "true") {
            var QueryString = $(this).attr('href').split('#')[1];            
                if (QueryString == 'CP_StickyNav_1') {
                    $('.cp-sticky-nav .tab-menu[data-active="true"]').attr('tabindex', '0').focus();
                }            
        }
    });
    
    $('.open-popup-video').each(function(){
    if($(this).attr('data-player-type')=="oneplayer"){
        //$(this).attr('href','javascript:void(0);');
        $(this).attr('data-js-dialog-show','default-lightbox');
    }
});
});
...
$(window).load(function () {
    ...
    $('.CP_AccordionWithTable .accordion-main-panel .subitems button').each(function () {
        if ($(this).text() == "") {
            $(this).remove();
        }
    });
...
/*348854*/
function slickstate() {
    $('.main-content .slick-arrow').each(function () {
        if ($(this).hasClass('slick-disabled')) {
            var currentslickval;
            currentslickval = $(this).text();
            $(this).attr('aria-label', currentslickval + ' disabled');
        } else {
            var currentsiblingval;
            currentsiblingval = $(this).text();
            $(this).attr('aria-label', currentsiblingval);
        }
    });
}
/*START **As part of July monthly audit of AT bug fix 350494** */
$(window).on("resize", function () {
    if (window.matchMedia("(max-width: 767px)").matches) {
        $('.cp-sticky-nav #stickyNotice').attr('aria-live','assertive');
        if ($(".cp-sticky-nav .tabs nav").attr('aria-hidden', 'false')) {
            $(".cp-sticky-nav .tabs nav").attr('aria-hidden', 'true');
        }
        $(".cp-sticky-nav nav ul li a").each(function () {
        ...
        $('.cp-sticky-nav #stickyNotice').removeAttr('aria-live');
    }
});
 /*END*/
 ...
 // Bug - 352285 
$(document).ready(function(){
    $("#SQL_Table_3Col_1_With_Filters .exp_elem_list").keydown(function(e) {
    ...
    $(document).ready(function(){
        ...
        // $(".custom-popup-background").css("position", "relative");
        // $(".page-frame-popup-background").css("position", "relative");
